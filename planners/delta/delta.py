"""
DELTA — LLM-to-PDDL Planning Pipeline (Original)

Original DELTA implementation adapted for HEART experiments.
Runs as a standalone CLI or is called programmatically by delta_planner.py.

Changes from original DELTA (https://github.com/clear-nus/delta):
  - llm/llm.py: Added OpenAI API client (original used Azure OpenAI)
  - prompt.py: Minor adjustments for HEART scene graph format
  - data/example.py: Extended with 40 task examples for 3 HEART scenes
  - data/scene_graph.py: Extended with 3 HEART scene graphs + robot configs
"""
