...
(define (domain microwave_apple)
    ...
    ; place_in: put item into open container. As result, item is IN container (item_in).
    (:action place_in
        ...
        :effect (and (item_in ?i1 ?i2) (not(agent_loaded ?a)) (not(agent_has_item ?a ?i1)))
    )
    ...
    ; turn_on: must be closed first
    (:action turn_on
        :parameters (?a - agent ?i - item ?r - room)
        :precondition (and
            (agent_at ?a ?r) (item_at ?i ?r) (item_accessible ?i)
            (item_turnable ?i) (item_closed ?i) (item_turned_off ?i)
            (not(agent_loaded ?a)))
        :effect (and (not(item_turned_off ?i)) (item_turned_on ?i))
    )
    ...
    (:action turn_off
        ...
        :precondition (and
            (agent_at ?a ?r) (item_at ?i ?r) (item_accessible ?i)
            (item_turnable ?i) (item_turned_on ?i)
            (not(agent_loaded ?a)))
        :effect (and (not(item_turned_on ?i)) (item_turned_off ?i))
    )
